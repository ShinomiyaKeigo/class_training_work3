...
static_assert(CAN_VALVE_HPP_INCLUDED==3.3,"CanValve version does not match. Check the latest version of the file on github.（CanValveクラスのcppとhppのバージョンが一致しません。githubで最新バージョンのファイルを確認してください。）");

CanValve::CanValve(Can &can,int canId,bool orderEnable){
	runSetupOnce=0;
	can.addHandler(this);

	this->canId=canId+0x280;

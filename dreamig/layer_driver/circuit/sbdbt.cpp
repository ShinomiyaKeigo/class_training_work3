...
float Sbdbt::getValueLx(){
	if(getStickLx() > 0)return floatlimit(-1.f, (float)getStickLx() / 64, 1.f);
	else if(getStickLx() < 0)return floatlimit(-1.f, (float)getStickLx() / 63, 1.f);
	return 0;
}
float Sbdbt::getValueLy(){
	if(getStickLy() > 0)return floatlimit(-1.f, (float)getStickLy() / 64, 1.f);
	else if(getStickLy() < 0)return floatlimit(-1.f, (float)getStickLy() / 63, 1.f);
	return 0;
}
...
//	左回転角度:+
//	右回転角度:-
float Sbdbt::getStickLangle(){return -atan2f(ly(), lx());}
float Sbdbt::getStickRangle(){return -atan2f(ry(), rx());}

//--- 0~+180
//	左回転角度
float Sbdbt::getStickLangle2(){
	if(getStickLangle() < 0)return getStickLangle()+2*M_PI;
	return getStickLangle();
}
float Sbdbt::getStickRangle2(){
	if(getStickRangle() < 0)return getStickRangle()+2*M_PI;
...
float Sbdbt::getAccelx(){
	...
	if(value > MID_ACCEL_X){
		angle = ((value - MAX_ACCEL_X) / (MAX_ACCEL_X - MID_ACCEL_X) + 1) * (M_PI / 2);
	}else if(value < MID_ACCEL_X){
		angle = ((MIN_ACCEL_X - value) / (MIN_ACCEL_X - MID_ACCEL_X) - 1) * (M_PI / 2);
	}else{
		angle = 0;
	}
	//return value;
...
float Sbdbt::getAccely(){
	...
	if(value > MID_ACCEL_Y){
		angle = ((value - MAX_ACCEL_Y) / (MAX_ACCEL_Y - MID_ACCEL_Y) + 1) * (M_PI / 2);
	}else if(value < MID_ACCEL_X){
		angle = ((MIN_ACCEL_Y - value) / (MIN_ACCEL_Y - MID_ACCEL_Y) - 1) * (M_PI / 2);
	}else{
		angle = 0;
	}
...
float Sbdbt::getAccelx2(){
	...
	value = (float)_order[ANALOG_ACCEL_X_L] + ((float)(_order[ANALOG_ACCEL_X_H] << 8));
	angle = (value - MIN_ACCEL_X) / ((MAX_ACCEL_X - MIN_ACCEL_X) / M_PI);
...
float Sbdbt::getAccely2(){
	...
	value = (float)_order[ANALOG_ACCEL_Y_L] + ((float)(_order[ANALOG_ACCEL_Y_H] << 8));
	angle = (value - MIN_ACCEL_Y) / ((MAX_ACCEL_Y - MIN_ACCEL_Y) / M_PI);
...
float Sbdbt::getValueAccelx(){
	...
	if(value > 0){
		if(value > (M_PI / 2))return 1;
		return value / (M_PI / 2);
	}else if(value < 0){
		if(value < -1 * M_PI / 2)return -1;
		return value / (M_PI / 2);
	}
	return 0;
}
...
float Sbdbt::getAngleAccel(){
	return atan2f(getValueAccelx(), getValueAccely())*-1 + M_PI;
}
...
//--- option
 ...
 float Sbdbt::start(){return getDigitalStart();}
 float Sbdbt::select(){return getDigitalSelect();}
...
//--- stick
float Sbdbt::lx(){
	if(fabsf(_tolerance.stick.lx) < fabs(getValueLx()))return getValueLx();
	return 0;
}
float Sbdbt::ly(){
	if(fabsf(_tolerance.stick.ly) < fabs(getValueLy()))return getValueLy();
	return 0;
}
int Sbdbt::l3(){return getDigitalL3();}
float Sbdbt::rx(){
	if(fabsf(_tolerance.stick.rx) < fabs(getValueRx()))return getValueRx();
	return 0;
}
float Sbdbt::ry(){
	if(fabsf(_tolerance.stick.ry) < fabs(getValueRy()))return getValueRy();
	return 0;
}

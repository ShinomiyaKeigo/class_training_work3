...
void Init_CANi(int can_num,uint8_t CAN_Mode,GPIO_TypeDef *tx_port,uint16_t tx_pin,GPIO_TypeDef *rx_port,uint16_t rx_pin){
...
//	printf("%x,%d",(int)data->StdId,data->DLC);
//	for(int i=0;i<data->DLC;i++)printf(",%d",data->Data[i]);
//	printf("\n");
	CAN_Transmit(CAN1, data);
}

void send_can2(CanTxMsg *data){
	CAN_Transmit(CAN2, data);
}

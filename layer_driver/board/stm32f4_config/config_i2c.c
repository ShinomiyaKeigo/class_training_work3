...
//	GPIO_InitTypeDef GPIO_InitStructure;
	...
	Init_port(GPIO_Mode_AF,GPIOxScl,sclPin,GPIO_PuPd_NOPULL,GPIO_OType_OD);
	Init_port(GPIO_Mode_AF,GPIOxSda,sdaPin,GPIO_PuPd_NOPULL,GPIO_OType_OD);

	if(use_i2c == I2C1){
	//	GPIO_Init(GPIOxScl, &GPIO_InitStructure);
		//GPIO_Init(GPIOxSda, &GPIO_InitStructure);
		GPIO_PinAFConfig(GPIOxScl, Pin_select_source(sclPin), GPIO_AF_I2C1);
		GPIO_PinAFConfig(GPIOxSda,  Pin_select_source(sdaPin), GPIO_AF_I2C1);
	}else if(use_i2c == I2C2){
		//GPIO_Init(GPIOxScl, &GPIO_InitStructure);
		//GPIO_Init(GPIOxSda, &GPIO_InitStructure);
		GPIO_PinAFConfig(GPIOxScl, Pin_select_source(sclPin), GPIO_AF_I2C2);
		GPIO_PinAFConfig(GPIOxSda, Pin_select_source(sdaPin), GPIO_AF_I2C2);
	}else if(use_i2c == I2C3){
//		GPIO_Init(GPIOxScl, &GPIO_InitStructure);
	...
	if(use_i2c==I2C1){
		...
		NVIC_InitStructure.NVIC_IRQChannel = I2C3_EV_IRQn;//|I2C2_ER_IRQn;
	}

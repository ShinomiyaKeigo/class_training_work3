...
float  Read_duty(TIM_TypeDef* TIMx,uint8_t ch){
	...
	if(ch==1)			return (float)TIMx->CCR1/TIMx->ARR;
	else if(ch==2) 	return (float)TIMx->CCR2/TIMx->ARR;
	else if(ch==3)		return (float)TIMx->CCR3/TIMx->ARR;
	else if(ch==4)		return (float)TIMx->CCR4/TIMx->ARR;
	else						return 0;
}
